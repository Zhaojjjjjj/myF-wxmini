...
<?php
// WebSocket服务端实现
// 需要先安装依赖：composer require ratchet/pawl ratchet/rfc6455
...
class WebSocketServer implements MessageComponentInterface
{
    ...
    public function onOpen(ConnectionInterface $conn)
    {
        ...
        // 检查房间是否存在且有效
        $room = Db::table('rooms')->where('id', $roomId)->where('status', 'active')->find();
        if (!$room) {
            $conn->send(json_encode(['type' => 'error', 'msg' => '房间不存在或已关闭']));
            $conn->close();
            return;
        }
